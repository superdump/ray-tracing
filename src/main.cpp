...
scene random_scene(float aspect) {
    int n = 500;
    hitable **list = new hitable*[n + 1];
    hitable **lights = new hitable*[n + 1];
    ...
    return scene {
        .cam = camera(lookfrom, lookat,
            ...
            0.0f, 1.0f),
        .world = new bvh_node(list, i, 0.0, 1.0),
        .lights = new bvh_node(lights, l, 0.0, 1.0),
    };
}

scene two_spheres(float aspect) {
    texture *checker = new checker_texture(
        new constant_texture(vec3(0.2f, 0.3f, 0.1f)),
        new constant_texture(vec3(0.9f, 0.9f, 0.9f))
    );
    ...
    return scene {
        ...
        .lights = new hitable_list(&list[2], 1),
    };
}

scene two_perlin_spheres(float aspect) {
...
scene earth(std::string tex, float aspect) {
    int nx, ny, nn;
    ...
    return scene {
        ...
        .lights = new hitable_list(&list[1], 1),
    };
}

scene simple_light(float aspect) {
    ...
    return scene {
        ...
        .lights = new hitable_list(&list[2], 2),
    };
}

scene cornell_box(float aspect) {
    hitable **list = new hitable*[8];
    hitable **lights = new hitable*[8];
    ...
    list[i++] = new translate(new rotate_y(
        new box(vec3(0.0f, 0.0f, 0.0f), vec3(165.0f, 330.0f, 165.0f), aluminium), 15.0f), vec3(265.0f, 0.0f, 295.0f));
    ...
    return scene {
        ...
        .lights = new hitable_list(lights, l),
    };
}

scene cornell_smoke(float aspect) {
    hitable **list = new hitable*[8];
    hitable **lights = new hitable*[8];
...
scene cornell_balls(float aspect) {
    hitable **list = new hitable*[9];
    hitable **lights = new hitable*[9];
    int i = 0;
    int l = 0;
...
scene cornell_final(std::string tex, float aspect) {
    hitable **list = new hitable*[30];
    hitable **lights = new hitable*[30];
...
scene final(std::string tex, float aspect) {
    int nb = 20;
    hitable **list = new hitable *[30];
    hitable **lights = new hitable *[30];
    hitable **boxlist = new hitable *[10000];
    ...
    return scene {
        ...
        .lights = new hitable_list(lights, ll),
    };
}

scene get_scene(std::string s, std::string texture, float aspect) {
    if (s == "random_scene") {
        return random_scene(aspect);
    } else if (s == "two_spheres") {
        return two_spheres(aspect);
    } else if (s == "two_perlin_spheres") {
        return two_perlin_spheres(aspect);
    } else if (s == "earth") {
        return earth(texture, aspect);
    } else if (s == "simple_light") {
        return simple_light(aspect);
    } else if (s == "cornell_box") {
        return cornell_box(aspect);
    } else if (s == "cornell_smoke") {
        return cornell_smoke(aspect);
    } else if (s == "cornell_balls") {
        return cornell_balls(aspect);
    } else if (s == "cornell_final") {
        return cornell_final(texture, aspect);
    } else if (s == "final") {
        return final(texture, aspect);
    }
}

vec3 color(const ray& r, const hitable *world, hitable *lights, int nb, uint64_t& ray_count, int depth, uint32_t& state) {
    ++ray_count;
    hit_record hrec;
    if (world->hit(r, 0.001f, std::numeric_limits<float>::max(), hrec, state)) {
        scatter_record srec;
        vec3 emitted = hrec.mat_ptr->emitted(r, hrec, hrec.u, hrec.v, hrec.p);
        if (depth < nb && hrec.mat_ptr->scatter(r, hrec, srec, state)) {
            if (srec.is_specular) {
                return srec.attenuation * color(srec.specular_ray, world, lights, nb, ray_count, depth + 1, state);
            } else {
                hitable_pdf plight(lights, hrec.p);
                mixture_pdf p(&plight, srec.pdf_ptr);
                ray scattered = ray(hrec.p, p.generate(state), r.time());
                float pdf_val = p.value(scattered.direction(), state);
                delete srec.pdf_ptr;
                return emitted
                    + srec.attenuation
...
struct ParallelTaskSet : enki::ITaskSet {
    ParallelTaskSet(int nx, int ny, int ns, int nb, camera& c, hitable *w, hitable *l, uint8_t *i, unsigned int *fb_)
            : nx(nx), ny(ny), ns(ns), nb(nb), cam(c), world(w), lights(l), image(i), fb(fb_) {
        m_SetSize = ny;
        ray_count = 0;
        lines_complete = 0;
    }
        ...
        for (uint32_t j = range.start; j < range.end; ++j) {
            ...
            for (int i = 0; i < nx; ++i) {
                ...
                for (int s = 0; s < ns; ++s) {
                    ...
                    col += de_nan(color(r, world, lights, nb, local_ray_count, 0, state));
                }
                col /= float(ns);
                // gamma correction
...
int main(int argc, const char** argv)
{
    ...
    g_TS.Initialize();
    ParallelTaskSet task(nx, ny, ns, nb, scene.cam, scene.world, scene.lights, image, fb);
